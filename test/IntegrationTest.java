...
public class IntegrationTest {
    ...
    private static final FeaturePhone bertsNokia = new FeaturePhone(
            "Berts Nokia 6150");
    private static final Smartphone charliesBlackberry = new Smartphone(
            "Charlies BlackBerry");
    ...
    public IntegrationTest() {
        ...
        annasEmailAccount.setLoginCredentials(new UsernamePassword(annasEmail,
                "a99a"));
        annasComputer.openMailProgram().setAccountFor(MessageType.EMAIL,
                annasEmailAccount);
        annasIPhone.openMailProgram().setAccountFor(MessageType.EMAIL,
                annasEmailAccount);
        ...
        annasIPhone.getMessageClient().setAccountFor(MessageType.SMS,
                annasSwisscomAbo);
        annasIPhone.getMessageClient().setAccountFor(MessageType.MMS,
                annasSwisscomAbo);
        ...
        bertsEmailAccount.setLoginCredentials(new UsernamePassword(bertsEmail,
                "b11b"));
        bertsComputer.openMailProgram().setAccountFor(MessageType.EMAIL,
                bertsEmailAccount);
        ...
        charliesEmailAccount.setLoginCredentials(new UsernamePassword(
                charliesEmail, "bb10"));
        charliesBlackberry.openMailProgram().setAccountFor(MessageType.EMAIL,
                charliesEmailAccount);
        ...
        charliesSunriseAbo.setLoginCredentials(new IMEI(charliesBlackberry
                .getImei()));
        charliesBlackberry.getMessageClient().setAccountFor(MessageType.SMS,
                charliesSunriseAbo);
        charliesBlackberry.getMessageClient().setAccountFor(MessageType.MMS,
                charliesSunriseAbo);
    ...
    public synchronized void testRegisterAtServer() {
        Status status = gmail.register("dave@gmail.com", new UsernamePassword(
                "dave@gmail.com", "a99a"));
        assertEquals("Register Dave", status.getCode(), 200);
        assertTrue("Dave is registered at Google",
                gmail.doesAccountExist("dave@gmail.com"));
        ...
        assertTrue("Anna ist bei Swisscom",
                swisscom.doesAccountExist(annasTelNr));
        assertTrue("Charlie ist bei Sunrise",
                sunrise.doesAccountExist(charliesTelNr));
    }
    ...
    public synchronized void testLogin() {
        ...
        assertFalse("Benutzer ist nicht mehr eingeloggt",
                gmail.isUserLoggedIn(annasEmail));
        List<Message> poll = ua.receiveMessages();
        assertNull(
                "Ausgeloggte Benutzer dürfen nicht mehr Nachrichten empfangen",
                poll);
        status = ua.sendMessage(new EmailMessage());
        assertTrue("Ausgeloggte Benutzer dürfen nicht mehr Nachrichten senden",
                status.getCode() != 200);

        // wrong password
        ServerProxy proxy = internet.lookup("gmail.com");
        ServerSocket socket = proxy.login(annasEmail, new UsernamePassword(
                annasEmail, "a11a"), null);
        ...
        status = ua.login();
        assertEquals("Login funktioniert", status.getCode(), 200);
        assertTrue("Benutzer ist eingeloggt",
                sunrise.isUserLoggedIn(charliesTelNr));
        ua.logout();
    }
    ...
    public synchronized void testSendMessageWithoutReminder() {
        ...
        assertEquals("Email von ist gesetzt", email.getFrom(),
                annasEmailAccount.getAddress());
        email.addRecipient(bertsEmail);
        email.setSubject("Email Test");
        email.setMessage("Dies ist ein Test");
        outlook.submit(email);

        assertTrue("Message ist auf dem Server",
                gmail.getMessagesForUser(bertsEmail).contains(email));
    ...
    public synchronized void testSendEmailAcrossDomains() {
        ...
        outlook.submit(email);
        assertTrue("Message ist auf dem Server",
                gmx.getMessagesForUser(charliesEmail).contains(email));
        bbMail.login();
    ...
    public synchronized void sendSMSMessage() {
        ...
        // assertTrue("Message ist auf dem Server",
        // sunrise.getMessagesForUser(charliesTelNr).contains(sms));
        // SMS ist push
        List<Message> messages = bbMail.getMessagesFromInbox();
        try {
            Thread.sleep(100);
