...
public class MessageDialog extends JDialog {
     ...
     */
    public MessageDialog(Message message, MessageType messageType,
            MessageClient messageClient, boolean draft) {
    ...
    public void fillComponentsWithMessageProperties() {
        ...
        if (message instanceof MessageWithSubjectAndAttachment) {
            subjectField.setText(((MessageWithSubjectAndAttachment) message)
                    .getSubject());

            for (Attachment file : ((MessageWithSubjectAndAttachment) message)
                    .getAttachments()) {
    ...
    private void configureFrame() {
                    ...
                    for (File file : attachementFiles) {
                        if (file.isDirectory()) {
                            JOptionPane
                                    .showConfirmDialog(
                                            null,
                                            "Sie haben einen Ordner ausgewählt, sie müssen eine Datei auswählen",
                                            "Ordner nicht erlaubt",
        ...
        guiManager.setX(0).setY(0).setWidth(1).setWeightX(1)
                .setComp(new JLabel("An"));

        if (messageType.instance() instanceof MessageWithSubjectAndAttachment) {
            guiManager.setX(1).setY(0).setWidth(6).setWeightX(6)
                    .setFill(GridBagConstraints.HORIZONTAL).setComp(toField);
            guiManager.setX(7).setY(0).setWidth(1).setWeightX(1).setHeight(2)
                    .setComp(reminderPanel);
            guiManager.setX(0).setY(1).setWidth(1).setWeightX(1)
                    .setComp(new JLabel("Betreff"));
            guiManager.setX(1).setY(1).setWidth(6).setWeightX(6)
                    .setFill(GridBagConstraints.HORIZONTAL)
                    .setComp(subjectField);

            guiManager.setX(0).setY(2).setWidth(1).setWeightX(1)
                    .setComp(new JLabel("Anhang"));
            guiManager.setX(1).setY(2).setWidth(6).setWeightX(8)
                    .setFill(GridBagConstraints.HORIZONTAL)
                    .setComp(attachmentPanel);
            guiManager.setX(7).setY(2).setWeightX(1).setHeight(1)
                    .setFill(GridBagConstraints.HORIZONTAL)
                    .setComp(searchButton);
        } else {

            guiManager.setX(1).setY(0).setWidth(6).setWeightX(6)
                    .setFill(GridBagConstraints.HORIZONTAL).setComp(toField);
            guiManager.setX(7).setY(0).setWidth(1).setWeightX(1)
                    .setComp(reminderPanel);
        }

        guiManager.setX(0).setY(3).setWidth(8).setWeightX(8).setWeightY(25)
                .setHeight(5).setScrollPanel().setComp(messageTextField);

        guiManager.setX(0).setY(8).setWidth(2).setWeightX(2)
                .setFill(GridBagConstraints.HORIZONTAL).setComp(cancelButton);
        guiManager.setX(2).setY(8).setWidth(2).setWeightX(2)
                .setFill(GridBagConstraints.HORIZONTAL).setComp(sendButton);
        guiManager.setX(4).setY(8).setWidth(2).setWeightX(2)
                .setFill(GridBagConstraints.HORIZONTAL).setComp(saveButton);
        guiManager.setX(6).setY(8).setWidth(2).setWeightX(2)
                .setFill(GridBagConstraints.HORIZONTAL).setComp(reminderButton);
    ...
    private Message buildMessage() {
        ...
        if (messageType.instance() instanceof MessageWithSubjectAndAttachment) {
            ((MessageWithSubjectAndAttachment) message).setSubject(subjectField
                    .getText());
            ((MessageWithSubjectAndAttachment) message)
                    .fillAttachements(attachmentPanel.getAttachments());
        }
