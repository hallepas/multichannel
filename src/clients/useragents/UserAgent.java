...
/**
 * Der User Agent kümmert sich um das Einloggen am Server sowie das schicken und
...
public abstract class UserAgent {
    ...
    public Status login(ClientProxy client) {
        ...
        if (ip == null) {
            ...
            return new Status(404, "Server " + account.getServer()
                    + "not found.");
        }
        ...
        if (socket != null) {
            ...
            log.fine("Successfully logged in at " + account.getServer() + "."
                    + socket);
            return new Status(200, "Successfully logged in at "
                    + account.getServer());
        }
        log.fine("Could not log in at " + account.getServer() + ".");
        return new Status(405, "Login at server " + account.getServer()
                + " failed");
    }
    ...
    public Status sendMessage(Message message) {
        ...
        log.info("Tried to send message " + message
