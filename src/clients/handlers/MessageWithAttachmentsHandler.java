...
public abstract class MessageWithAttachmentsHandler extends MessageHandler {
    ...
    public void saveAttachments(MessageWithSubjectAndAttachment message,
            String path) throws IOException {
        if (!Files.isDirectory(Paths.get(path))) {
            throw new IOException("Path " + path + " is not a directory.");
        }
        for (Attachment attachment : ((MessageWithSubjectAndAttachment) message)
                .getAttachments()) {
    ...
    public void addAttachment(MessageWithSubjectAndAttachment message,
            String path) throws IOException {
